# Apigee Edge and Auth0 Integration
...
## TOC
* [Save Auth0's Authorization Code in Apigee Edge](#apigee-auth0-external-authorization)
* [Save Auth0's Access Token in Apigee Edge](#save-auth0s-access-token-in-apigee-edge)
...
### Update edge.json
You should update the following entries in the `auth0-oauth/edge.json` and `auth-oauth-save-token/edge.json` files. Just use a find and replace to update all the values shown below.  
...
# Save Auth0's Authorization Code in Apigee Edge
...
Use the developer `john@example.com` and the app named `auth0-save-token-app`.
...
Copy the following command into your browser.  Be sure to update the [`audience`](#auth0-api-identifier), `{org}`, `{env}` and `{clientID}` parameters.

**If you include the [`audience parameter`](#auth0-api-identifier) then Auth0 will return a JWT as the access token.**
```
